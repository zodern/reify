...
function Entry(id) {
  ...
  this._getterNames = null;
  this._exportNames = null;
}
...
Ep._getGetterNames = function () {
  if (!this._getterNames) {
    this._getterNames = Object.keys(this.getters);
  }

  return this._getterNames;
}

Ep.finalize = function () {
  this._finalized = true;
}

Ep._getExportNames = function () {
  if (!this._finalized) {
    return Object.keys(this.module.exports);
  }

  if (!this._exportNames) {
    this._exportNames = Object.keys(this.module.exports);
  }
...
Ep.runGetters = function (names) {
  ...
  for (var i = 0; i < nameCount; ++i) {
    ...
    if (value !== GETTER_ERROR && changed(this._lastValues, name, value)) {
      this._changed = this._changed || Object.create(null);
...
function syncExportsToNamespace(entry, names) {
  ...
  if (! utils.getESModule(exports)) {
    ...
    if (changed(entry._lastValues, 'default', exports)) {
      entry._changed = entry._changed || Object.create(null);
      entry._changed.default = true;
    }
  ...
  names.forEach(function (key) {
    ...
    if (! hasOwn.call(entry.getters, key) &&
        ...
        (isOwn || hasOwn.call(exports, key))) {
        var desc = Object.getOwnPropertyDescriptor(exports, key);
        var compareValue = typeof desc.get === 'function' ? desc.get : desc.value; 
        if (changed(entry._lastValues, key, compareValue)) {
          Object.defineProperty(entry.namespace, key, desc);
          entry._changed = entry._changed || Object.create(null);
          entry._changed[key] = true;
        }
    }
  });
}
...
Ep.runSetters = function (getterNames, runNsSetter) {
  ...
  if (this._changed !== null) {
    var setterNames = Object.keys(this._changed);
    setterNames.push('*');
    if (!(utils.getESModule(exports) &&
      "default" in exports)) {
        setterNames.push('default');
      }
    this._changed = null;
    forEachSetter(this, setterNames, void 0, runSetter);
  }
...
function callSetterIfNecessary(setter, name, value, callback) {
  ...
  if (!setter.initialized && value === undefined) {
    call = false;
  }
